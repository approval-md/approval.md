...
import { usdOrZero } from "../src/core/money.js";
...
/**
 * An amount as the gate writes it since APRV-121: the canonical decimal string.
 *
 * The tests below name amounts as numbers because that is how a reader thinks
 * about money; this is the one place they become the spelling the write
 * boundary admits. A bare number handed to `appendEvent` is refused now, which
 * is the whole point of the change, so the conversion is explicit and lives
 * here rather than being scattered through the cases.
 */
function usd(cost: number): string {
  return usdOrZero(cost);
}
...
function grant(actionClass: string, cost: number | null, ts: string, key?: string): EventInput {
  ...
  if (cost !== null) payload["est_cost_usd"] = usd(cost);
  return {
    ts,
...
function nonConsuming(
  ...
  return {
    ...
    payload: { class: actionClass, est_cost_usd: usd(cost) },
  };
}
