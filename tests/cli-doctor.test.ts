...
test("doctor: every check passes or skips on a healthy environment", async () => {
  ...
  assert.deepEqual(
    ...
    // The roster moved to tests/doctor-rows.ts, with its per-row rationale
    // intact, because the README states how many rows doctor prints and which
    // of them skip in a fresh directory. One exported list makes that prose
    // derive from this assertion instead of drifting beside it; the docs guard
  ...
  assert.deepEqual(
    ...
    [
      ...
      "skip",
      // codex-auto-reviewer passes: the fixture log carries no
      // `audit.question_preempted`, so nothing other than this gate is
      // recorded as having answered one of its questions. A pass and not a
      // skip for the reason autonomy-alias is one: the question was asked and
...
test("doctor: --json emits exactly one object with the frozen shape", async () => {
  ...
  // 31: APRV-338 appended `pending-sign-off` (which protected files still
  // carry SPEC.md's pending-sign-off marker with no record ratifying their
  // current bytes) and APRV-342 appended `attested-policy-on-main` (whether
  // the attested policy hash matches the APPROVAL.md blob at origin/main).
  // APRV-335 appended `autonomy-alias` (which rules still write the
