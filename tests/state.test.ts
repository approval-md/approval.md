...
import { headOf, readVerifiedRecords } from "../src/core/state.js";
...
test("an absent log reads as an empty log with a null head", () => {
  const result = readVerifiedRecords(join(scratch, "nowhere", "events.jsonl"));
...
test("a clean log yields its records and the head an append can be pinned to", () => {
  const logPath = buildLog(3);
  const result = readVerifiedRecords(logPath);
  assert.equal(result.ok, true);
...
test("a mutated record is log-corrupt, not silently trusted", () => {
  const logPath = buildLog(3);
  ...
  const result = readVerifiedRecords(tampered);
...
test("a deleted record is log-corrupt: the chain notices the gap", () => {
  const logPath = buildLog(3);
  const spliced = `${logPath}.spliced`;
  const lines = readFileSync(logPath, "utf8").split("\n").filter((line) => line.length > 0);
  writeFileSync(spliced, `${[lines[0], lines[2]].join("\n")}\n`);

  const result = readVerifiedRecords(spliced);
  assert.equal(result.ok, false);
  if (!result.ok) assert.equal(result.code, "log-corrupt");
});

test("a torn tail keeps its own code: nothing is repaired here", () => {
  const logPath = buildLog(3);
  ...
  const result = readVerifiedRecords(torn);
  assert.equal(result.ok, false);
  if (!result.ok) {
...
test("an unopenable log is log-unreadable — an I/O fact, never an accusation", {
