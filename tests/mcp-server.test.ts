...
test("mcp: the exclusions are agent-facing verbs, each with a stated reason", () => {
  ...
  assert.deepEqual([...EXCLUDED_VERBS.keys()].sort(), [
    "consume",
    "hook claude-code",
    "hook codex",
    "hook cursor",
    ...
    // lands. APRV-358 gave `hook grok` the registry entry it had been missing
    // since APRV-243 and this list the row that follows from it;
