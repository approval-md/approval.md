...
test("tamper before the cached head, preserving size and head bytes, is refused", () => {
  ...
  assert.deepEqual(cache.stats, { hits: 0, misses: 2 }, "the prefix hash must discard the entry");
});
...
test("tamper in the appended suffix is refused on the resumed walk", () => {
  ...
  assert.deepEqual(cache.stats, { hits: 1, misses: 1 }, "an intact prefix is reused");
});
...
test("a shrunken log reports its own head, never the remembered one", () => {
  ...
  assert.deepEqual(cache.stats, { hits: 0, misses: 2 }, "a shorter file discards the entry");
});
...
test("caches are per-instance, and eviction costs correctness nothing", () => {
  ...
  assert.deepEqual(isolated.stats, { hits: 0, misses: 0 });
});
