...
| `gunzip` | gunzip | (any) | read.shell for `-c`/`--stdout` and `-t`/`--test`; files.write.workspace otherwise, because the default form REPLACES the file it names (`-k` keeps the input and still creates the output); files.delete.out_of_scope ¶ when the named path is outside the workspace (APRV-397) |
