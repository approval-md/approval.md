...
| `gunzip` | gunzip | (any) | read.shell for `-c`/`--stdout` and `-t`/`--test`; files.write.workspace otherwise, since the default form replaces the file it names; files.delete.out_of_scope ¶ outside the workspace (APRV-397) |
