...
Two bearer credentials come from the launch environment, and authorization is per verb. `APPROVAL_SERVE_AGENT_TOKEN` opens the hook and an allowlist of what a harness under oversight needs in order to ask and to act on a grant: `instructions`, `hook_classify`, `register`, `request`, `wait`, `withdraw`, the payload builders, `policy_check`, `policy_test`, `log_verify` and `gate_status`. `APPROVAL_SERVE_TENANT_TOKEN` opens everything else, including the verbs that return records (`log_tail`, `log_export`, `queue`, `status`) and the ones that touch the host (`run`, `sandbox`, the adapters, `log_advance`). The agent credential never reads the log it is judged by, and a verb added to the registry is tenant-scoped until somebody widens the allowlist. The export carries `APPROVAL.md`, the log, the payload bytes behind every `payload_hash` and the projections, and nothing under `.approval/keys`, `.approval/env`, `.approval/daemon` or the vault. The server binds loopback unless `--listen` names another interface in full and `--allow-non-loopback` is passed as well; it terminates no TLS, so put a proxy you control in front of it. See [the reference](cli-reference.md#serve).
