...
| `daemons` | The daemon instance ids allowed to append, for a daemon one party hosts for another; absent means no restriction. |
...
| Hermes Agent | `approval hook hermes` | Live-probe evidence on `main` at `118984d7`: `fail_closed: true` blocks a hook crash, a hook timeout and unparseable output, which makes this the only harness besides Claude Code whose hook is enforcement rather than a backstop. Below that build the key is ignored silently, so the version floor and `approval doctor`'s row matter. Requires explicit hook registration, `hooks_auto_accept` where there is no TTY, and an absolute working directory on every gated call. |
...
Two bearer credentials come from the launch environment, and authorization is per verb. `APPROVAL_SERVE_AGENT_TOKEN` opens the hook and a five-verb allowlist of what a harness under oversight needs in order to ask and to be answered: `instructions`, `hook_classify`, `request`, `wait` and `withdraw`. `APPROVAL_SERVE_TENANT_TOKEN` opens everything else, including the verbs that return records or gate state (`log_tail`, `log_export`, `log_verify`, `gate_status`, `queue`, `status`), the ones that answer what the policy does (`policy_check`, `policy_test`), the ones that touch the host (`run`, `sandbox`, the adapters, `log_advance`, `register`, whose positional is a host path), and both payload builders, since one takes a file on the daemon's machine and the other spends the host's own AgentMail key on ids the caller chose. The agent credential never reads the log it is judged by, a verb added to the registry is tenant-scoped until somebody widens the allowlist, and no caller of any scope names a path: the store flags are pinned from the launch configuration, a positional beginning with a dash is refused because the verb would read it as a flag, the agent may not use a path-typed flag at all, and the tenant's are confined to the store. Which flags are path-typed comes from the registry, so a new one is confined on the day it appears. A verb call and a hook call answer with one body carrying `exit_code`, `stdout` and `stderr`, so a refusal can never be read as a permission. The export carries `APPROVAL.md`, the log, the payload bytes behind every `payload_hash` and the projections, follows no link of either kind (a symlink or a second hard name for one inode refuses the whole export), and carries nothing under `.approval/keys`, `.approval/env`, `.approval/daemon` or the vault. The server binds loopback unless `--listen` names another interface in full and `--allow-non-loopback` is passed as well; it terminates no TLS, so put a proxy you control in front of it. See [the reference](cli-reference.md#serve).
