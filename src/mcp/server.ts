...
export const EXCLUDED_VERBS: ReadonlyMap<string, string> = new Map([
  ...
  [
    "hook grok",
    "it reads one Grok Build PreToolUse event from STDIN and its registry input schema has nowhere to put that event. On a stdio server, stdin is the JSON-RPC stream: a tool that read it would eat the protocol. It is also the wrong shape for MCP — a harness that can call tools calls `request` and `wait` directly.",
  ],
  [
    "hook muse",
    "it reads one Muse Code PreToolUse or PostToolUse event from STDIN and its registry input schema has nowhere to put that event. On a stdio server, stdin is the JSON-RPC stream: a tool that read it would eat the protocol. It is also the wrong shape for MCP — a harness that can call tools calls `request` and `wait` directly.",
  ...
  [
    "hook hermes",
    "it reads one Hermes Agent pre_tool_call or post_tool_call event from STDIN and its registry input schema has nowhere to put that event. On a stdio server, stdin is the JSON-RPC stream: a tool that read it would eat the protocol. It is also the wrong shape for MCP — a harness that can call tools calls `request` and `wait` directly.",
  ],
]);
  ...
  const server = new Server(
    { name: "approval-md", version: SPEC_VERSION },
    {
      ...
      instructions:
        options.guest === true
          ? GUEST_INSTRUCTIONS
          : "approval.md gates agent actions that touch the world. The tools here are the AGENT surface only: declare an action in a task envelope, register it, request it, wait for a decision, then run it. There is deliberately no grant, reject, revoke, attest or vault tool — those record a human's authority and are not available to a client of this server (SPEC.md §11). One tool is not gated at all: `journal_write` appends free text to a local file, is never classified or routed through policy, can be neither approved nor denied, and writes no event — it is there so you can say what an exit code cannot (that you are complying and think this is wrong, that something reads as odd, that you are stuck). The operator reads it; nothing written there changes any verdict. One tool runs the other way: `values` prints the operator's stated preferences from APPROVAL.md, which is guidance rather than policy — it grants nothing and changes no verdict — and reading it before you start is how you know what they value. `feedback` is that channel in the other direction: what the operator said about work that already happened, human-authored GUIDANCE that is not policy: it grants nothing, forbids nothing, and changes no verdict, sampling probability or budget. Call `instructions` first; it prints the full guide.",
    },
  );
