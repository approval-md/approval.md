...
const VERBS: VerbSpec[] = [
...

  {
    ...
    subcommand: "prepare",
    purpose:
      "Write a fresh inert review bundle for a constrained Codex host. It writes only --output, activates nothing, and cannot establish enforcement.",
    human_only: true,
    human_only_note:
      "Host preparation is an operator workflow and is excluded with the whole codex family from the broad agent MCP catalog.",
    input: input({
      flags: {
    ...
    output: object(
      { ok: { const: true }, inert: { const: true }, output: STRING, files: arrayOf(STRING), manifest: OPEN_OBJECT },
      ["ok", "inert", "output", "files", "manifest"],
    ),
    error: ERROR_SCHEMA,
  ...
  },
  {
    ...
    subcommand: "setup",
    purpose:
      "Verify a prepared Codex bundle's exact file set, hashes, manifest, and generated bytes. Success means internally consistent and inert, never installed or ready.",
    human_only: true,
  ...
  },
  {
    ...
    subcommand: "doctor",
    purpose:
      "Fail-closed inspection of the manifest, platform, pinned version declarations, principals, path separation, symlinks, ownership, modes, invocation, broker and runner. It executes no manifest binary and APRV-325.1 always reports runtime versions unchecked and not ready.",
    human_only: true,
