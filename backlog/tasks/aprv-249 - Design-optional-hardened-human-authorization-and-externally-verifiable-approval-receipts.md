...
Repair the receipt design against refreshed main in /private/tmp/approval-receipt-design-repair on codex/receipt-design-repair. Parent owns security review, protected prose application and delivery; Sol prepares only a scratch reviewed patch. Separate full-body signed authorization receipts from retrospective checkpoint witnesses; require complete canonical suffix records and independently pinned verification keys; defer witness runtime for disclosure/size; order sender attribution before quorum. Apply exact reviewed proposal only through primary policy.edit.design gate, then verify document consistency and required repository checks. This task remains design-only; no receipt runtime, new event, credentials or live ZZZ configuration is implied.
<!-- SECTION:PLAN:END -->
...
2026-09-09 Astra review during the user-authorized full open-work wave: keep design In Progress. The proposed phase-1 C2 checkpoint signature authenticates a chain prefix, not the new receipt body described in section 5.2. Audience, challenge, scope, sub_key and expiry cannot be added at export and treated as signed. Section 8 promises audience/challenge binding without specifying how those fields enter checkpoint-covered bytes, and negative tests 4/5/19 conflict with accepting an existing checkpoint signature as the C2 binding. AC3 and AC6 reopened for this concrete contract gap. Resolve with an explicitly distinct witness format that derives every authenticated field from covered records/payload bytes and labels unbound fields informational, or require a separate per-receipt signature. A list of hashes alone cannot prove an interior grant is chained to the signed head without the intervening authenticated record material. No receipt runtime or stronger ZZZ identity assurance has been implemented or claimed. Phase implementation still requires the separate authorization stated by this task; the general cleanup does not silently convert a design-only task into credential infrastructure.
