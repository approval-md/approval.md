...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 After token validation the verb long-polls until a message arrives or a stated deadline passes; no Enter is required
- [ ] #2 Every getUpdates still carries no offset and allowed_updates [message]; the existing tests asserting that still pass
- [ ] #3 On timeout, output includes pending_update_count and webhook status from getWebhookInfo and names the bot username to check
